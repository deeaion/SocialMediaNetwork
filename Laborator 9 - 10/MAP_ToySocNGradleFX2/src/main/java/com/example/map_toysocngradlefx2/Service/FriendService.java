...
import com.example.map_toysocngradlefx2.Domain.DTOs.FriendDTO;
...
public class FriendService extends Service implements Observable<SocialNetworkChangeEvent>{
    public Set<User> set;
    ...
    public boolean addFriendship(String username1, String username2) {
       ...
       notifyObservers(new SocialNetworkChangeEvent(ChangeEventType.ADDFRIEND,null));

        return true;
    }
    public boolean modifyFriendship(String username1,String username2,LocalDateTime time,FriendshipStatus status)
    {
        User user1=findbyUsername(username1);
        User user2=findbyUsername(username2);
        Friendship found=findFriendship(user1,user2);
        Friendship newFr=new Friendship(user1,user2,time,status);
    ...
    public List<User> getFriendshipsOfUsersByStatus(String username, FriendshipStatusType statusType)
    {
        User user=findbyUsername(username);
        if(user==null)
        {
        ...
        return StreamSupport.stream(friendships.spliterator(),false).
                filter(friend->{
                    if (friend.getStatus().equals (statusType.toString())) {
                        return true;
                    } else return false;
                }).
                map(friendship -> {
                ...
                .map(o -> (User) o).collect(Collectors.toList());
    }
    public List<User> getPendingRequests(String username)
    {
        ...
        return StreamSupport.stream(friendships.spliterator(),false).
                filter(friend->{
                    if (friend.getStatus().equals (FriendshipStatus.PENDING.toString())) {
                        return true;
                    } else return false;
                }).
                map(friendship -> {
